...
/*   Updated: 2017/11/18 19:25:56 by vle-gal          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char* ft_dstrz(ssize_t d, t_struct * para)
{
	...
	// d < 0 ? less = 1 && (d = d * -1) : 0;
	len = 0;
	d == -4594967295? (t_d = 4594967294) && (d = 4594967294) && (para->d++) : (t_d = d);
	while (t_d > 9)
	{
		t_d /= 10;
		len++;
...
void ft_d(t_struct * para, char* d)
{
	...
	para->p_ == 0 && para->s_ == 1 && d[0] != '-' && para->d_ == -1 ? ft_putchar(' '):0;
	para->p_ == 0 && para->s_ == 1 && d[0] != '-' && para->d_ == -1 ? para->res++ :0;
	// printf("%d, %d, %c, %d, %d\n", para->plus, para->nbr, d[0], para->less, para->dot);
	para->p_ == 1  && (para->nbr == -1 || para->l_ == 1) && d[0] != '-'  && para->d_ == -1? ft_putchar('+') : 0;
	para->p_ == 1  && (para->nbr == -1 || para->l_ == 1) && d[0] != '-'  && para->d_ == -1? para->res++ : 0;
	len = ft_strlen(d);
	if (para->d_ != -1)
...
void ft_d_main(t_struct * para, va_list args)
{
	...
	para->f_one == '\0' && para->chara == 'd' ? d = ft_dstr(va_arg(args, int), para) : 0;
