...
/*   Updated: 2017/11/18 19:29:18 by vle-gal          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_x_draw(t_struct * para, char* x)
{
	...
	para->h_ == 1? (x[1] = 'x') && (x[para->nbr - (len - 1)] = '0'): 0;
	para->chara == 'X' && para->z_ == 1 && para->nbr > len && para->h_ == 1?
	(x[1] = 'X') && (x[para->nbr - (len - 1)] = '0'): 0;
	(para->chara == 'x' || para->chara == 'X') && para->h_ == 1 && para->z_ == 1?
	 x[0] = '0': 0;
	ft_putstr(x);
}
