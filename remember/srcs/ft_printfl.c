...
/*   Updated: 2017/11/18 19:31:50 by vle-gal          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void ft_flags(const char* format, t_struct *para)
{
				...
				format[para->i] == 'z' || format[para->i] == '#')
	{
		...
		format[para->i] <= '9' && format[para->i] > '0' ? para->nbr = ft_flag_bis(format, para): 0;
...
static void ft_para(const char* format, t_struct *para, va_list args)
{
	...
	format[para->i] == 'i' || format[para->i] == 'I' ? ft_d_main(para, args): 0;
